...
sub vcl_recv {
	...
	if ( req.url == "/" || req.url ~ "\.(html|htm|css|js|txt|xml|svg)(\?[a-z0-9=\.]+)?$" || req.url ~ "^/produktxxx/"  ) {
...
sub vcl_backend_response {
	...
	if ( bereq.url == "/" || bereq.url ~ "\.(html|htm|css|js|txt|xml|svg)(\?[a-z0-9=]+)?$" || bereq.url ~ "^/produktxxx/" ) {
