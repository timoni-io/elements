...
sub vcl_recv {
	...
	if ( req.method == "GET" ) {
		if ( req.url == "/" || req.url ~ "\.(html|htm|css|js|txt|xml|svg|jpg|png)(\?[a-z0-9=\.]+)?$" || req.url ~ "^/produkt/" || req.url ~ "^/wp-content/uploads/" || req.url ~ "^/sklep/" || req.url ~ "^/kategoria-produktu/"  ) {
...
sub vcl_backend_response {
	...
	if ( bereq.method == "GET" ) {
		if ( bereq.url == "/"  || bereq.url ~ "\.(html|htm|css|js|txt|xml|svg|jpg|png)(\?[a-z0-9=\.]+)?$" || bereq.url ~ "^/produkt/" || bereq.url ~ "^/wp-content/uploads/" || bereq.url ~ "^/sklep/" || bereq.url ~ "^/kategoria-produktu/" ) {
